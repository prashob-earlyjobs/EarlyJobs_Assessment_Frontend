...
// import { Award, Calendar, User, CheckCircle, Download } from "lucide-react";
...
//   return (
//     <div className="flex flex-col items-start gap-2 bg-white shadow-md p-4 rounded-lg">
...
//   <Badge className={`bg-white text-green-700 px-3 ${isPDFGenerating ? "": "py-1"} rounded-full flex items-center gap-1 border border-green-600`}>
//     <span className={`text-sm font-medium ${isPDFGenerating && "mb-[1rem]"}`}>Overall Score:</span>
//     <span className={`text-sm font-bold text-green-600 ${isPDFGenerating && "mb-[1rem]"}`}>{score}/10</span>
//   </Badge>
//   <Badge className="bg-white text-green-700 px-3 py-1 rounded-full flex items-center gap-1 border border-green-600">
//     <span className={`text-sm font-medium ${isPDFGenerating && "mb-[1rem]"}`}>Communication:</span>
//     <span className={`text-sm font-bold text-green-600 ${isPDFGenerating && "mb-[1rem]"}`}>{commScore}/10</span>
//   </Badge>
//   <Badge className="bg-white text-green-700 px-3 py-1 rounded-full flex items-center gap-1 border border-green-600">
//     <span className={`text-sm font-medium ${isPDFGenerating && "mb-[1rem]"}`}>Proctoring:</span>
//     <span className={`text-sm font-bold text-green-600 ${isPDFGenerating && "mb-[1rem]"}`}>{proctScore}/10</span>
//   </Badge>
// </div>
...
useEffect(() => {
  ...
  const handleDownloadAndSend = async () => {
    try {
      await generateAndDownloadPDF();
      await sendPDFToBackend();
      setShowShareOptions(true); // Show share options after successful download and upload
    } catch (error) {
